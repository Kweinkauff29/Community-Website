-- SNEAK IDX Phase 7.3C3A: repair legacy consumer foreign-key target drift.
--
-- Early staging copies of the five tables below referenced
-- sneak_sites(site_id), but the canonical site primary key is sneak_sites(id).
-- Refuse to rebuild when consumer data exists so this migration can never
-- silently discard account or activity state. A populated legacy environment
-- must use an environment-specific data-preserving repair instead.
