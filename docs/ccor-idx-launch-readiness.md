# CCOR IDX Phase 7.4A Launch Readiness

Current release gate: **PASS**. Implementation, staging deployment, automated regression, live API/detail verification, remote D1 checks, and mandatory actual-browser consumer/Admin/Member acceptance all pass.

Build: `2026.08.31.7.4a`
...
| Entitlements | Generic table existed, but duplicated fail-open logic | Central fail-closed authority | GrowthZone reconciliation remains manual |
...
- GrowthZone in 7.4A: manual source/reference only
...
- Consumer login: optional capability; real transactional delivery must be verified separately.
- Saved-search email alerts: optional and currently not ready. This does not mark base IDX down.
...
- Actual-browser acceptance: `527/527` checks using local Google Chrome through Playwright against staging at 1440×900, 1024×768, and 390×844.
- Consumer browser matrix: five Residential, two Rental, two Land, and two Commercial listings at every viewport, including seven samples with more than 20 photos. Checks covered contained imagery, counts, bounded 12-thumbnail window/downloads, navigation, responsive detail hierarchy, full remarks/attribution, category facts, zero-value suppression, rapid A→B race safety, card entry, Compare/Recent state, no image errors, no page errors, and no Bridge traffic.
...
- Member/C2B browser regression: authenticated Clients navigation, four KPI cards, email search, sorting, Client Detail, Saved Homes, Saved Searches & Alerts, Activity Timeline, Inquiries, protected APIs, modal fit, and no horizontal clipping at all three viewports.
...
| Consumer Worker | Optional for base search | Worker healthy; no provider secrets; simulated adapter | `MAILJET_API_KEY`, `MAILJET_SECRET_KEY`, `EMAIL_FROM` | `/health`, real inbox test | Buyer magic links do not arrive | Configure provider and verify sender/inbox |
...
| GrowthZone process | Yes for paid entitlement operations | Manual/reference only | No API secret in 7.4A | Staff entitlement review | Stale entitlement state | Reconcile manually; automate in 7.4B |
...
- Member magic link: `MAILJET_API_KEY` and `MAILJET_SECRET_KEY` names are present on the staging Member Worker, so current requests select the real Mailjet adapter. Public responses never return the raw token. A current real inbox delivery/consumption was not verified in 7.4A.
- Consumer magic link: the staging Consumer Worker has no provider secret names, so requests select the simulated staging adapter. This is not real delivery and is not launch-ready for buyer login.
- Saved-search alerts: fail-closed shared Mailjet provider plus unsubscribe signing secret. Current declared state is Mailjet not configured, signing secret not configured, delivery not ready, and real delivery not verified.
...
## Production plan for Phase 7.4B (document only)
...
6. Reconcile GrowthZone to generic entitlements, retaining manual override/audit procedures.
